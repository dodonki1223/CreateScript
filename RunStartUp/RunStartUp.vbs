...
Sub Main()
    ...
    '--------------------------------------
    ' PortableAppsのシンボリックリンク作成を実行
    '--------------------------------------
    objShell.Run runDrive & "\Tools\CreateScript\RunStartUp\CreateSymLinksForPortableApps.vbs"

    '--------------------------------------
    ' PortableAppsのシンボリックリンク作成を実行
    '--------------------------------------
    objShell.Run runDrive & "\Tools\CreateScript\RunStartUp\CreateSymLinksForAutoHotKeyTool.vbs"
...
Function AddShortCutFile(ByRef pFileInfo,ByVal pRunDrive,ByVal pOrchisDirectory)
    ...
    pFileInfo.Add "アプリと機能.lnk"                          , "%windir%\explorer.exe"                                                               & "|" & pRunDrive & "\Tools\Shortcuts\Windows\設定\"               & "|" & "ms-settings:appsfeatures"                 & "|" & "%WinDir%\System32\imageres.dll, 251"
    ...
    pFileInfo.Add "ネットワークとインターネット.lnk"          , "%windir%\explorer.exe"                                                               & "|" & pRunDrive & "\Tools\Shortcuts\Windows\設定\"               & "|" & "ms-settings:network"                      & "|" & "%WinDir%\System32\wlanpref.dll, 5"
